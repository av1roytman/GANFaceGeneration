...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            utils.spectral_norm(nn.ConvTranspose2d(100, 16384, 4, 1, 0, bias=False)),
            nn.BatchNorm2d(16384),
            nn.ReLU(True),
            # state size.
            
            utils.spectral_norm(nn.ConvTranspose2d(16384, 8192, 4, 2, 1, bias=False)),
            nn.BatchNorm2d(8192),
            nn.ReLU(True),
            # state size
            
            utils.spectral_norm(nn.ConvTranspose2d(8192, 4096, 4, 2, 1, bias=False)),
            nn.BatchNorm2d(4096),
            nn.ReLU(True),
            # state size.
            
            utils.spectral_norm(nn.ConvTranspose2d(4096, 2048, 4, 2, 1, bias=False)),
            nn.BatchNorm2d(2048),
            nn.ReLU(True),
            # state size. 

            utils.spectral_norm(nn.ConvTranspose2d(2048, 1024, 4, 2, 1, bias=False)),
            nn.BatchNorm2d(1024),
            nn.ReLU(True),
            # state size. 
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            utils.spectral_norm(nn.Conv2d(1024, 2048, 3, stride=2, padding=1)),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.Dropout(0.5), # Dropout Layer
            # state size. 
            
            utils.spectral_norm(nn.Conv2d(2048, 4096, 3, stride=2, padding=1)),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.Dropout(0.5), # Dropout Layer
            # state size.
            
            utils.spectral_norm(nn.Conv2d(4096, 8192, 3, stride=2, padding=4)),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.Dropout(0.5), # Dropout Layer
            # state size. 
            
            utils.spectral_norm(nn.Conv2d(8192, 1, kernel_size=4, stride=1, padding=0)),
            nn.LeakyReLU(0.2, inplace=True),
            
            utils.spectral_norm(nn.Conv2d(8192, 16384, 3, stride=2, padding=1)),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout(0.5), # Dropout Layer
            # state size. 
            
            utils.spectral_norm(nn.Conv2d(16384, 1, kernel_size=4, stride=1, padding=0)),
            nn.LeakyReLU(0.2, inplace=True),
...
model_base = '../model_states/10-layer'
torch.save(netG.state_dict(), os.path.join(model_base, 'Gen-10Layer-128x128-SAGAN.pth'))
...
plt.savefig(os.path.join(base, '16Layer-128x128-SAGAN.png'))
plt.close(fig)
...
plt.ylabel("Loss")
plt.legend()
plt.savefig(os.path.join(base, 'Loss_16Layer-128x128-SAGAN.png'))
plt.close()
