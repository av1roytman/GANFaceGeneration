...
class Generator(nn.Module):
    def __init__(self, noise_dim=512, embed_dim=1024, ff_dim=2048, dropout=0.1):
        ...
        self.blocks_stage1 = nn.Sequential(*[TransformerBlock(embed_dim, ff_dim, dropout) for _ in range(5)])

        # Stage 2: Upsampling and transformer blocks
        self.upsample_stage2 = nn.Upsample(scale_factor=2, mode='nearest')
        self.blocks_stage2 = nn.Sequential(*[TransformerBlock(embed_dim, ff_dim, dropout) for _ in range(4)])

        # Stage 3: Pixel shuffle and transformer blocks
        self.pixel_shuffle_stage3 = nn.PixelShuffle(2)
        self.blocks_stage3 = nn.Sequential(*[GridTransformerBlock(256, ff_dim, 32, dropout) for _ in range(4)])

        # Stage 4: Pixel shuffle and grid transformer blocks
        self.pixel_shuffle_stage4 = nn.PixelShuffle(2)
        self.blocks_stage4 = nn.Sequential(*[GridTransformerBlock(64, ff_dim, 64, dropout) for _ in range(4)])

        # Stage 5: Pixel shuffle and grid transformer blocks
        self.pixel_shuffle_stage5 = nn.PixelShuffle(2)
        self.blocks_stage5 = nn.Sequential(*[GridTransformerBlock(16, ff_dim, 128, dropout) for _ in range(4)])
